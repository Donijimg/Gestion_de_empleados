...
def save_empleado(request):
    ...
    new_empleado = Empleado.objects.create(nombre_empleado=nombre_empleado, documento=documento, correo_electronico=correo_electronico , fabrica=fabrica, puesto=puesto)
...
def accion_recuperar_empleado(request):
    return render(request, 'recuperar_empleado.html', {})
...
def accion_eliminar_empleado(request):
    return render(request, 'eliminar_empleado.html', {})

def eliminar_pais(request):
    pais_id = request.POST['id']
    pais=Pais.objects.get(id=pais_id)
    pais.delete()
    return HttpResponse("pais eliminado")


def accion_eliminar_pais(request):
    return render(request, 'eliminar_pais.html', {})
